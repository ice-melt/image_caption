...
def demo(opt):
    ...
    for step in range(1000):
        ...
        entry = {'image_id': '%d.jpg' %(img_id[0]), 'caption': sents[0]}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_from', type=str, default='save/normal_coco_1024_adam', help='')
    parser.add_argument('--load_best_score', type=int, default=1,
                    help='Do we load previous best score when resuming training.')     
    parser.add_argument('--id', type=str, default='',
                    help='an id identifying this run/job. used in cross-val and appended when writing progress files')
    parser.add_argument('--image_path', type=str, default='data/coco/images',
    ...
    if args.start_from is not None:
        ...
        with open(info_path,'rb') as f:
            infos = cPickle.load(f,encoding='iso-8859-1')
            opt = infos['opt']
    ...
    if args.start_from != None:
        ...
        if os.path.isfile(os.path.join(args.start_from, 'histories_'+opt.id+'.pkl')):
            with open(os.path.join(args.start_from, 'histories_'+opt.id+'.pkl'),'rb') as f:
                histories = cPickle.load(f,encoding='iso-8859-1')
    ...
    print('saving...')
    json.dump(predictions, open('visu.json', 'w',encoding="utf-8"))
