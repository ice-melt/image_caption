...
class InferenceWrapperBase(object):
  ...
  def build_model_eval(self, model_config):
    ...
    Returns:
      model: The model object.
    """
    tf.logging.fatal("Please implement build_model_eval in subclass")
  ...
  def build_graph_from_config_eval(self, model_config, checkpoint_path):
    """Builds the inference graph from a configuration object.
    ...
    """
    tf.logging.info("Building model.")
    self.build_model_eval(model_config)
    saver = tf.train.Saver()
